...
angular.module('angularfire2App')
  .service('gpChartInterfaces', function () {
    ...
    this.FlatChart = {
		Info: function (points, idSelector, valueSelector, nameSelector, categorySelector ){ // perhaps rather than just saving 3 things, this should accept a single object, throw error if it lacks the things, but be okay with extra things, like a php interface.
		...
		// 	this.categorySelector = clone(categorySelector); // this is for if we use one chart rather than 2.
		}
	};

	// this.Point = function( id, value, name, category ){
	// 	return {
	// 		id: id,
	// 		value: value,
	// 		name: name,
